...
const ProductCard: React.FC<ProductProps> = ({
  ...
  return (
    <Card className="w-full rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 bg-white">
      <CardHeader shadow={false} floated={false} className="relative h-[200px] md:h-[400px]">
        <div
          className="w-full h-[200px] md:h-[400px] bg-gray-200 front-slider-hero"
      ...
      <CardBody className="px-4 py-3">
        ...
        >
          <Typography
            variant="h5"
            color="blue-gray"
            className="font-bold tracking-tight text-sm md:text-lg"
          >
            {name}
          </Typography>
        </Link>
      ...
      {!addtocart && (
        ...
        >
          <div className="flex-col md:flex">
            ...
            <Typography className="md:ml-2 line-through">
              {regularPrice} Rs
            </Typography>
          </div>
          ...
          <div>
            <Button
              ...
              size="sm"
              onClick={() => addcart(id)}
              className="mr-2 text-color-by-logo-2 border-color-by-logo-2 mt-2 sm:mt-0 text-xs px-2 py-1 md:text-md md:px-4 md:py-2"
            >
              Add to cart
            </Button>
          </div>
        ...
        <CardFooter className={`flex flex-col justify-between px-4 py-3 border-t border-gray-200 border-t border-gray-200`}>
          ...
          <>
            <div className="flex-col justify-start sm:justify-end mt-2 sm:mt-0 md:flex-row">
              ...
              <Button variant="outlined" size="sm" onClick={() => addcart(id)} className="mt-2 md:mt-0 text-xs px-2 py-1 md:text-md md:px-4 md:py-2">
                Remove
              </Button>{" "}
            </div>
          </>
