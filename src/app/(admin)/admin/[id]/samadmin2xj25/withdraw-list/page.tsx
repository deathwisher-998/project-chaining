...
export default function Productpage() {
  ...
  async function getAdminuser() {
    try {
      setloading((e) => 1);
      const response: any = await withDrawpendinglist().then((res) => res);
  ...
  async function addReferenceuser(data: any) {
    try {
      if (data) {
        setloading((e) => 1);
        const response = await withDrawUpdatestatus(data).then(
          (res: any) => res
        );
        if (response) {
          getAdminuser();
            ...
            <div className="p-5">
              ...
              <div className="mt-5 mb-5">
                ...
                {referralsubmit && !refferralCodevalue && (
                  <div>
                    <p className="text-red-900 text-sm">Please select status</p>
                  </div>
                )}
              </div>
